...
b0=ea_open_vol([directory,options.prefs.b0]);
...
trkcmd=[DSISTUDIO,' --action=trk',...
    ...
    ' --dt_threshold=0.2',...
    ' --smoothing=0.25',...
    ' --step_size=',num2str(mean(b0.voxsize)/2),...
    ' --turning_angle=75',...
    ' --output=',ea_path_helper([directory,ftrbase,'.trk'])];
...
ea_trk2ftr([directory,ftrbase,'.trk']);
copyfile([directory,ftrbase,'.mat'],[directory,ftrbase,'_dup.mat']);
ea_ftr2trk([directory,ftrbase,'_dup.mat'], [directory,options.prefs.b0])
