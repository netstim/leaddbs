...
if ischar(options) % return name of method.
    switch options
        case 'prompt'
            varargout{1}='Siobhan MAGeT Brain-like Segmentation/Normalization DISTAL atlas (Chakravarty 2013, Ewert 2016)';
...
for peer=1:length(peerfolders)
    ...
    if ~exist([subdirec,'MAGeT',filesep,'warpreceives',filesep,poptions.patientname,'2sub.nii.gz'],'file') || reforce
        ...
        ea_ants_nonlinear_se(sptos,spfroms,[subdirec,'MAGeT',filesep,'warpreceives',filesep,poptions.patientname,'.nii'],weights,metrics,options);
