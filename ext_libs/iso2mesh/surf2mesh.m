...
if(isempty(cmdopt))
  system([' "' mcpath('tetgen') exesuff '" -A -T1e-20 -q2 -a -Y ' num2str(maxvol) ' ' moreopt ' "' mwpath('post_vmesh.poly') '"']);
else
  system([' "' mcpath('tetgen') exesuff '"  -A -T1e-10 -q2 -a -Y ' cmdopt ' "' mwpath('post_vmesh.poly') '"']);
end
