function varargout = ea_cat_run(job)
...
% ______________________________________________________________________
% Christian Gaser
% $Id: cat_run.m 1285 2018-02-28 07:19:05Z gaser $
...
if isfield(job,'nproc') && job.nproc>0 && (~isfield(job,'process_index'))  
  ...
  for i=1:job.nproc
    ...
    matlab_cmd = sprintf(...
        ['"global cprintferror; cprintferror=1; addpath %s %s %s %s %s;load %s; ' ...
         'global defaults; defaults=spm12def; clear defaults; '...
         'global cat; cat=cat12def; clear cat; ea_cat_run(job); "'],...
