function [min_bound_per_contact, max_bound_per_contact, S] = ea_get_currents_per_contact(min_conc,max_conc,min_segm,max_segm, reconst, side, check_S)
    ...
    if any(contains(concentric4,el_models{1}))
        ...
        if check_S == 1
            ...
            load([ea_getearoot,'ext_libs',filesep,'PathwayTune',filesep,'S_4contact.mat']);
            if min_conc < 0.0
                if length(side) == 2
    ...
    elseif any(contains(concentric8,el_models{1}))
        ...
        if check_S == 1
            ...
            load([ea_getearoot,'ext_libs',filesep,'PathwayTune',filesep,'S_8contact_conc.mat']);
            if min_conc < 0.0
                if length(side) == 2
    ...
    elseif any(contains(segmented8,el_models{1}))
        ...
        if check_S == 1
            load([ea_getearoot,'ext_libs',filesep,'PathwayTune',filesep,'S_8contact_segm.mat']);
