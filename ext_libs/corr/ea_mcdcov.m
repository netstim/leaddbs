function [rew,raw]=ea_mcdcov(x,varargin)
...
[Pcl,Tcl,Lcl,rcl,centerXcl,cXcl] = ea_classSVD(data);
clasmean=cXcl;
        ...
        for i=1:nsamp
            ...
            if ~Hsets_ind
                ...
                elseif fine
                    ...
                    [P,T,L,r,centerX,meanvct] = ea_classSVD(data(index,:));
                    if r==0
                        ns = 1;
                        rew.center=meanvct;
                        rew.cov=zeros(p,p);
                    ...
                    elseif (r < p) & (r~=0)
                        ...
                        if count >= adjh
                            ...
                            while det(covmat) < exp(-50*p)
                                ...
                                [covmat,meanvct] = ea_updatecov(data(index,:),covmat,meanvct,data(setdiff(index1,index),:),[],1);
                                index = index1;
                            end
                        end
                    end
            ...
            if ~ns
                for j=1:csteps
                    ...
                    [P,T,L,r,centerX,meanvct] = ea_classSVD(data(obs_in_set,:));
                    if r==0
                        rew.center=meanvct;
                        rew.cov=zeros(p,p);
...
[P,T,L,r,centerX,cX] = ea_classSVD(data(bestset,:));
mah=ea_mahalanobis((data - repmat(cX,n,1))*P,zeros(size(P,2),1),'cov',L);
sortmah=sort(mah);
...
weights=mah<=cutoff.md^2;
raw.wt=weights;
[rew.center,rew.cov]=ea_weightmecov(data,weights);
[trcov,trcenter]=trafo(rew.cov,rew.center,med,mad,p);

% determination of Hfreq:
[telobs,indobs] = ea_greatsort(teller(1:(end - 1)));
...
if prod(sqrt(L)) < exp(-50*p)
    ...
    mah=ea_mahalanobis(data,rew.center,'cov',rew.cov);
    rew.flag=(mah <= cutoff.md^2);
    rew.rd=sqrt(mah);
end

rew.mahalanobis=sqrt(ea_mahalanobis(data,clmean,'cov',clcov));
rawo=raw;
reso=rew;
if options.classic==1
    classic.cov=clascov;
...
if ~part | final
    [dis,ind]=sort(ea_mahalanobis(dat,meanvct,'cov',covmat));
    mahsort=ind;
elseif fine
    [dis,ind]=sort(ea_mahalanobis(dat(obsingroup{end},:),meanvct,'cov',covmat));
    mahsort=obsingroup{end}(ind);
else
    [dis,ind]=sort(ea_mahalanobis(dat(obsingroup{k},:),meanvct,'cov',covmat));
