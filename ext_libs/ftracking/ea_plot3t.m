...
if ~(sum((size(linex)==size(liney)) & (size(linex)==size(linez))) == 2)
    error('Input variables x,y,z are not equal in length or no vectors')
end
if size(linex,1)>1
    linex=linex';
end
...
if (min(linel)/2.2)<bufdist
    bufdist=min(linel)/2.2;
end
...
function [a,b]=getab(normal)
    ...
    % if(sum(abs(cross(randomv,normal)))<0.001)
    %     randomv=[0.58 0.5774 0.56];
    % end
