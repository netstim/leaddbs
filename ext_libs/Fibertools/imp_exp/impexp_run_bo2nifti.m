function varargout = impexp_run_bo2nifti(cmd, varargin)
% Template function to implement callbacks for an cfg_exbranch. The calling
% syntax is
% varargout = impexp_run_bo2nifti(cmd, varargin)
% where cmd is one of
% 'run'      - out = impexp_run_bo2nifti('run', job)
%              Run a job, and return its output argument
% 'vout'     - dep = impexp_run_bo2nifti('vout', job)
%              Examine a job structure with all leafs present and return an
%              array of cfg_dep objects.
% 'check'    - str = impexp_run_bo2nifti('check', subcmd, subjob)
...
% 'defaults' - defval = impexp_run_bo2nifti('defaults', key)
...
%              impexp_run_bo2nifti('defaults', key, newval)
...
% 'run'      - @(job)impexp_run_bo2nifti('run', job)
% 'vout'     - @(job)impexp_run_bo2nifti('vout', job)
% 'check'    - @(job)impexp_run_bo2nifti('check', 'subcmd', job)
% 'defaults' - @(val)impexp_run_bo2nifti('defaults', 'defstr', val{:})
...
% $Id: impexp_run_bo2nifti.m,v 1.4 2010/07/01 09:28:16 glauche Exp $
    ...
    switch lower(cmd)
        case 'run'
            ...
            if isfield(job.srcdtdchoice,'srcdtdvar')
                ...
                if isfield(job.outname,'autoimg')
                    for k = 1:numel(job.srcdtdchoice.srcdtdstruct)
                        ...
                        outname{k}    = sprintf('%s.nii',strcat(n(1:end-3),'b0image'));
                    end
                end
            end
            ...
            for k = 1:numel(dtdStruct)
                ...
                if isfield(cmrStruct,'b0_image_struc')
                    [Vo errstr] = mrstruct_to_nifti(cmrStruct.b0_image_struc, fullfile(outdir{1}, ...
                        outname{k}), 'int16');
                    if ~isempty(errstr)
                        error(errstr);
                    end;
                else
                    error('The dtdstruct does not contain a b0-image.')
                end
                Vo = cell2mat(Vo);
