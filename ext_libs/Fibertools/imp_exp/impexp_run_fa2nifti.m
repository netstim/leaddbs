function varargout = impexp_run_fa2nifti(cmd, varargin)
% Template function to implement callbacks for an cfg_exbranch. The calling
% syntax is
% varargout = impexp_run_fa2nifti(cmd, varargin)
% where cmd is one of
% 'run'      - out = impexp_run_fa2nifti('run', job)
...
% 'check'    - str = impexp_run_fa2nifti('check', subcmd, subjob)
...
% 'defaults' - defval = impexp_run_fa2nifti('defaults', key)
...
%              impexp_run_fa2nifti('defaults', key, newval)
...
% 'run'      - @(job)impexp_run_fa2nifti('run', job)
% 'vout'     - @(job)impexp_run_fa2nifti('vout', job)
% 'check'    - @(job)impexp_run_fa2nifti('check', 'subcmd', job)
% 'defaults' - @(val)impexp_run_fa2nifti('defaults', 'defstr', val{:})
...
% $Id: impexp_run_fa2nifti.m,v 1.3 2010/07/01 09:28:16 glauche Exp $
    ...
    switch lower(cmd)
        case 'run'
            ...
            if isfield(job.srcdtdchoice,'srcdtdvar')
                ...
                if isfield(job.outname,'autoimg')
                    for k = 1:numel(job.srcdtdchoice.srcdtdstruct)
                        ...
                        outname{k}    = sprintf('%s.nii',strcat(n(1:end-3),'FAimage'));
                    end
                end
            end
            ...
            for k = 1:numel(dtdStruct)
                ...
                [FAmap, errstr] = dtdstruct_query(cmrStruct,'getFA');
                if ~isempty(errstr)
                    error(errstr);
                end
                [Vo errstr] = mrstruct_to_nifti(FAmap, fullfile(outdir{1}, ...
                                                                  outname{k}), 'float32');
                if ~isempty(errstr)
                    error(errstr);
                end
