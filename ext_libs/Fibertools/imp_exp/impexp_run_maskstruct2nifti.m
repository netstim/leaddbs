function varargout = impexp_run_maskstruct2nifti(cmd, varargin)
% Template function to implement callbacks for an cfg_exbranch. The calling
% syntax is
% varargout = impexp_run_maskstruct2nifti(cmd, varargin)
% where cmd is one of
% 'run'      - out = impexp_run_maskstruct2nifti('run', job)
%              Run a job, and return its output argument
% 'vout'     - dep = impexp_run_maskstruct2nifti('vout', job)
%              Examine a job structure with all leafs present and return an
%              array of cfg_dep objects.
% 'check'    - str = impexp_run_maskstruct2nifti('check', subcmd, subjob)
...
% 'defaults' - defval = impexp_run_maskstruct2nifti('defaults', key)
...
%              impexp_run_maskstruct2nifti('defaults', key, newval)
...
% 'run'      - @(job)impexp_run_maskstruct2nifti('run', job)
% 'vout'     - @(job)impexp_run_maskstruct2nifti('vout', job)
% 'check'    - @(job)impexp_run_maskstruct2nifti('check', 'subcmd', job)
% 'defaults' - @(val)impexp_run_maskstruct2nifti('defaults', 'defstr', val{:})
...
% Volkmar Glauche
% $Id: impexp_run_maskstruct2nifti.m,v 1.2 2010/07/01 09:28:16 glauche Exp $
    ...
    switch lower(cmd)
        case 'run'
            ...
            if isfield(job.srcchoice,'srcvar')
                ...
                % postpone read
                maskStruct = job.srcchoice.srcstruct;
            ...
            if numel(outname) ~= numel(maskStruct)
                nmrs = ceil(log10(numel(maskStruct)));
                [p n e] = fileparts(outname{1});
                on = sprintf('%s%%.0%dd%s',n,nmrs,e);
                for k = 1:numel(maskStruct)
                    outdir{k} = outdir{1};
            ...
            out.files = {};
            for k = 1:numel(maskStruct)
                if isstruct(maskStruct)
                    cmaskStruct = maskStruct(k);
                elseif iscellstr(maskStruct)
                    [cmaskStruct errstr] = maskstruct_read(maskStruct{k});
                    if ~isempty(errstr)
                        error('impexp:maskstruct2nifti',errstr);
                    end
                else
                    cmaskStruct = maskStruct{k};
                end
                % extract selected mask(s)
                [maxNo errstr] = maskstruct_query(cmaskStruct, 'maskNo');
                if ~isempty(errstr)
                    error('impexp:maskstruct2nifti',errstr);
                end
                if any(~isfinite(job.output.maskNo))
                    maskNo = 1:maxNo;
                else
                    maskNo = job.output.maskNo(job.output.maskNo<=maxNo);
                end
                [maskNames errstr] = maskstruct_query(cmaskStruct, 'maskNames');
                if ~isempty(errstr)
                    error('impexp:maskstruct2nifti',errstr);
                end
                for cmaskNo = maskNo
                    [cmask errstr]=maskstruct_query(cmaskStruct,'getMRMask', cmaskNo);
                    if ~isempty(errstr)
                        error('impexp:maskstruct2nifti',errstr);
                    end
                    [p n e] = fileparts(outname{k});
                    coutname = sprintf('%s-%s%s',n, genvarname(maskNames{cmaskNo}), e);
                    [Vo errstr]=mrstruct_to_nifti(cmask, fullfile(outdir{k}, ...
                        coutname), 'uint8');
                    if ~isempty(errstr)
                        error('impexp:maskstruct2nifti',errstr);
                    end
        ...
        case 'check'
            if ischar(varargin{1})
                ...
                switch subcmd
                    ...
                    case 'ismaskstruct'
                        % implement checks, return status string in variable str
                        if maskstruct_istype(subjob)
                            str = '';
                        else
                            str = 'Input is not a maskStruct variable.';
                        end
                    otherwise
