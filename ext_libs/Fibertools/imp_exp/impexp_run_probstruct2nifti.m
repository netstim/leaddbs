function varargout = impexp_run_probstruct2nifti(cmd, varargin)
% Template function to implement callbacks for an cfg_exbranch. The calling
% syntax is
% varargout = impexp_run_probstruct2nifti(cmd, varargin)
% where cmd is one of
% 'run'      - out = impexp_run_probstruct2nifti('run', job)
%              Run a job, and return its output argument
% 'vout'     - dep = impexp_run_probstruct2nifti('vout', job)
%              Examine a job structure with all leafs present and return an
%              array of cfg_dep objects.
% 'check'    - str = impexp_run_probstruct2nifti('check', subcmd, subjob)
...
% 'defaults' - defval = impexp_run_probstruct2nifti('defaults', key)
...
%              impexp_run_probstruct2nifti('defaults', key, newval)
...
% 'run'      - @(job)impexp_run_probstruct2nifti('run', job)
% 'vout'     - @(job)impexp_run_probstruct2nifti('vout', job)
% 'check'    - @(job)impexp_run_probstruct2nifti('check', 'subcmd', job)
% 'defaults' - @(val)impexp_run_probstruct2nifti('defaults', 'defstr', val{:})
...
% Volkmar Glauche
% $Id: impexp_run_probstruct2nifti.m,v 1.2 2010/07/01 09:28:16 glauche Exp $
    ...
    switch lower(cmd)
        case 'run'
            ...
            if isfield(job.output.outname,'outimg')
                ...
                outdir = {pwd};
                outname = {sprintf('%s.nii', job.output.mapType)};
            end
            if isfield(job.srcchoice,'srcvar')
                probStruct=job.srcchoice.srcvar;
            else
                % postpone read
                probStruct = job.srcchoice.srcstruct;
                if isfield(job.output.outname,'autoimg')
                    for k = 1:numel(job.srcchoice.srcstruct)
                        [outdir{k} n] = fileparts(job.srcchoice.srcstruct{k});
                        outname{k}    = sprintf('%s_%s.nii',n,job.output.mapType);
                    end
                end
            end
            if numel(outname) ~= numel(probStruct)
                nmrs = ceil(log10(numel(probStruct)));
                [p n e] = fileparts(outname{1});
                on = sprintf('%s%%.0%dd%s',n,nmrs,e);
                for k = 1:numel(probStruct)
                    outdir{k} = outdir{1};
            ...
            for k = 1:numel(probStruct)
                if isstruct(probStruct)
                    cprobStruct = probStruct(k);
                elseif iscellstr(probStruct)
                    [cprobStruct errstr] = probstruct_read(probStruct{k});
                    if ~isempty(errstr)
                        error(errstr);
                    end
                else
                ...
                % extract selected map, put it into temporary mrStruct
                % for conversion
                [map errstr]=probstruct_query(cprobStruct,'getMap', ...
                                              job.output.mapType);
                if ~isempty(errstr)
                    error(errstr);
                end
                cmrStruct = mrstruct_init('volume',map, ...
                                                   cprobStruct);
                [Vo errstr]=mrstruct_to_nifti(cmrStruct, fullfile(outdir{k}, ...
