...
if ~isempty(nameStr)
    ...
    res= reshape(ftrStruct.fiber{id}.roiName, [prod(size(ftrStruct.fiber{id}.roiName)) 1]);
else
    res= {};
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
rndIdx= find(rndTmp);
res= private_getVertex(ftrStruct.connectCell, ftrStruct.curveSegCell, idxF(rndIdx), type, transMy);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fiberStruct, errStr]= local_getInArea(ftrStruct, region, nameRef, not, txtHandle)
...
if ~endFlag
    for k= 1:length(idxF)
        ...
        ok_A= ~isempty(find(((roiVert(:, 1) == vertVc_A(1)) & (roiVert(:, 2) == vertVc_A(2)) & (roiVert(:, 3) == vertVc_A(3)))'));
        ...
        elseif (ok_A == bothFlag) % falls nicht suche ob Kurven ende in ROI
            ...
            ok_B= ~isempty(find(((roiVert(:, 1) == vertVc_B(1)) & (roiVert(:, 2) == vertVc_B(2)) & (roiVert(:, 3) == vertVc_B(3)))'));
            if ok_B
                count= count + 1;
                idxF_new(count)= k;
                if bothFlag == 1
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [idxF, errStr]= private_getFiberId(ftrStruct, nameStr)
errStr= ''; idxF= [];
