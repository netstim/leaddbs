function [GridDim,dowarn] = LeG_determine_griddim(elec)
...
for e = elec.mindigit:elec.maxdigit
  if ~isempty(LeG_match_str(elec.ElecLab, num2str(e))) % in case labels are 1, 2, 3 etc.
    labels_ordered{end+1,1} = [elec.ElecStr num2str(e)];
    pos_ordered(end+1, :) = elec.elecpos(LeG_match_str(elec.ElecLab, num2str(e)),:);
  elseif ~isempty(LeG_match_str(elec.ElecLab, num2str(e, ['%0' num2str(numel(elec.ElecLab{1})) 'd']))) % in case labels are 001, 002, 003 etc.
    labels_ordered{end+1,1} = [elec.ElecStr num2str(e, ['%0' num2str(numel(elec.ElecLab{1})) 'd'])];
    pos_ordered(end+1, :) = elec.elecpos(LeG_match_str(elec.ElecLab, num2str(e, ['%0' num2str(numel(elec.ElecLab{1})) 'd'])),:);
  else
    elec.cutout(end+1) = e;
...
if dowarn
%     msgbox('%s appears to be missing electrodes %s or have electrodes that are labeled using an unconventional numbering system', elec.ElecStr, num2str(elec.cutout));
    return;
end
