function ea_get_fiberstate_from_axonstate(ConnectomeName,ID_no_sub,stimdir,side,stimSetMode,native,convert_to_MNI,prob_PAM)
    ...
    [native_loc,stim_ID,~] = fileparts(stimdir);
    [stimulations_dir,~,~] = fileparts(native_loc);
    templateOutputBasePath = [stimulations_dir,filesep,'MNI152NLin2009bAsym',filesep,stim_ID,'/'];
    ...
    if ~isempty(axonState)
        for f=1:length(axonState)
            ...
            if settings.stimSetMode
                ...
                if startsWith(settings.connectome, 'Multi-Tract: ')
                    fiberActivation = [outputBasePath,subSimPrefix, 'fiberActivation_model-ossdbs_hemi-', sideLabel, '_tract-', tractName, '.mat'];
                else
                    fiberActivation = [outputBasePath,subSimPrefix, 'fiberActivation_model-ossdbs_hemi-', sideLabel, '.mat'];
                end
            end
            ...
            if convert_to_MNI==1 && options.native % Generate fiber activation file in MNI space
                ...
                if startsWith(settings.connectome, 'Multi-Tract: ')
                    ...
                    atlas_folder = [ea_getconnectomebase, 'dMRI_MultiTract', filesep, ConnectomeName];
                ...
                if settings.stimSetMode
                    ...
                    if startsWith(settings.connectome, 'Multi-Tract: ')
                        fiberActivationMNI = [templateOutputBasePath,subSimPrefix, 'fiberActivation_model-ossdbs_hemi-', sideLabel, '_tract-', tractName, '.mat'];
                    else
                        fiberActivationMNI = [templateOutputBasePath,subSimPrefix, 'fiberActivation_model-ossdbs_hemi-', sideLabel, '.mat'];
                    end
                end
