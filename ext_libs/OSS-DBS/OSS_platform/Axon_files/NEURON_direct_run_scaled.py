...
def conduct_parallel_NEURON(S_vector, population_name, last_point, N_index_glob, N_index, Ampl_scale,
                            t_steps, n_segments, dt, tstop, n_pulse, v_init, output):
...
def run_simulation_with_NEURON(d, S_vector, last_point, population_index, fib_diam, n_Ranvier,
                               N_models, Ampl_scale, n_processors, scaling_index,
                               neuron_array_name=None):
    # this script is solely for McIntyre2002 model
    ...
    paste_to_hoc_python3(n_Ranvier, paranodes1, paranodes2, axoninter, n_segments, d['v_init'], axon_dict["fiberD"],
                         axon_dict["para1_length"], axon_dict["para2_length"], axon_dict["ranvier_length"],
                         axon_dict["node_diameter"], axon_dict["axon_diameter"], axon_dict["para1_diameter"],
    ...
    if population_index == -1:  # only one population is simulated
        population_name = ''
        Vert_full_get = read_csv(os.environ['PATIENTDIR'] + '/Neuron_model_arrays/All_neuron_models.csv', delimiter=' ',
                                 header=None)  # get all neuron models
        Vert_full = Vert_full_get.values
        Vert_full = np.round(Vert_full, 8)

        Vert_get = read_csv(os.environ['PATIENTDIR'] + '/Neuron_model_arrays/Vert_of_Neural_model_NEURON.csv',
                            delimiter=' ', header=None)  # get only physiologically correct neuron models
        ...
        hf2 = h5py.File(os.environ['PATIENTDIR'] + '/Neuron_model_arrays/Vert_of_Neural_model_NEURON_by_populations.h5',
                        'r')
        lst = list(hf2.keys())
    ...
    Nodes_status = np.zeros((N_models * n_segments, 4),
                            float)  # Nodes_status will contain info whether the placed(!) axon was activated
    Nodes_status[:, :3] = Vert[:, :]
    ...
    while Neuron_index < N_models:
        ...
        while j_proc < n_processors and Neuron_index < N_models:
            first_axon_point = np.array([Vert[Neuron_index * n_segments, 0], Vert[Neuron_index * n_segments, 1],
                                         Vert[Neuron_index * n_segments, 2]])
            second_axon_point = np.array(
                [Vert[Neuron_index * n_segments + 1, 0], Vert[Neuron_index * n_segments + 1, 1],
                 Vert[Neuron_index * n_segments + 1, 2]])
            last_axon_point = np.array([Vert[Neuron_index * n_segments + n_segments - 1, 0],
                                        Vert[Neuron_index * n_segments + n_segments - 1, 1],
            ...
            processes = mp.Process(target=conduct_parallel_NEURON, args=(
            S_vector, population_name, last_point, neuron_global_index_array[Neuron_index], Neuron_index,
            Ampl_scale, t_steps_trunc, n_segments, dt, tstop, n_pulse, d['v_init'], output))
    ...
    Vert_full_status = np.zeros(Number_of_axons_initially,
                                int)  # has status of all neurons (-1 - was not placed, 0 - was not activated, 1 - was activated)
    ...
    Axon_status = np.zeros((N_models, 7),
                           float)  # x1,y1,z1,x2,y2,z2,status. Holds info only about placed neurons. Important: coordinates are in the initial MRI space!

    [Mx_mri, My_mri, Mz_mri, x_min, y_min, z_min, x_max, y_max, z_max, MRI_voxel_size_x, MRI_voxel_size_y,
     MRI_voxel_size_z] = np.genfromtxt(os.environ['PATIENTDIR'] + '/MRI_DTI_derived_data/MRI_misc.csv', delimiter=' ')
    ...
    for axon_i in range(Number_of_axons_initially):
        Axon_Lead_DBS[axon_i * n_segments:n_segments * (axon_i + 1), :3] = Vert_full[axon_i * n_segments:n_segments * (
                    axon_i + 1), :3] + shift_to_MRI_space
        Axon_Lead_DBS[axon_i * n_segments:n_segments * (axon_i + 1),
        3] = axon_i + 1  # because Lead-DBS number them from 1
        if axon_i in List_of_activated:
            Vert_full_status[axon_i] = 1
    ...
    if population_index == -1:  # only one population is simulated
        ...
        if d['Stim_side'] == 0:
            savemat(os.environ['PATIENTDIR'] + "/Results_rh/Axon_state_" + str(lst[population_index]) + "_" + str(
                scaling_index) + ".mat", mdic)
        else:
            savemat(os.environ['PATIENTDIR'] + "/Results_lh/Axon_state_" + str(lst[population_index]) + "_" + str(
                scaling_index) + ".mat", mdic)
    ...
    Nodes_status_MRI_space_only_activated = np.delete(Nodes_status_MRI_space,
                                                      np.where(Nodes_status_MRI_space[:, 3] == 0.0)[0], axis=0)
    ...
    if population_index == -1:
        ...
        if d['Stim_side'] == 0:
            ...
            np.savetxt(os.environ['PATIENTDIR'] + '/' + stim_folder + 'Network_status_' + str(scaling_index) + '.csv',
            ...
            np.savetxt(os.environ['PATIENTDIR'] + '/' + stim_folder + 'Activation_VAT_Neuron_Array_' + str(
                Activated_models) + '_' + str(scaling_index) + '.csv', Nodes_status_MRI_space_only_activated,
                       delimiter=" ")
        else:
            # np.savetxt(os.environ['PATIENTDIR']+'/Field_solutions/Activation/Last_run.csv', List_of_activated, delimiter=" ")
            np.savetxt(os.environ['PATIENTDIR'] + '/' + stim_folder + 'Last_run_' + str(scaling_index) + '.csv',
        ...
        if d['Stim_side'] == 0:
            np.savetxt(
                os.environ['PATIENTDIR'] + '/' + stim_folder + 'Last_run_in_' + str(lst[population_index]) + '_' + str(
            ...
            hf = h5py.File(os.environ['PATIENTDIR'] + '/' + stim_folder + 'Network_status_' + str(scaling_index) + '.h5',
                           'a')
