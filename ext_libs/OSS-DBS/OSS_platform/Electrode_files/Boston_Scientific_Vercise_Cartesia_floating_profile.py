...
Contact_3 = geompy.MakeRotation(Contact_2, OZ,
                                ang_betw_contacts)  # Boston has another notation (counter-clockwise from the top)
...
VolumeObject1 = [encap_outer_ROI, ROI, encap_inner_ROI, CV1, CV2, CV3, CV4, CV5, CV6, CV7,
                 CV8]  # Declare objects included to partition, encap_outer_ROI always @1st position
Volume_name1 = ['encap_outer_ROI1', 'ROI1', 'encap_inner_ROI1', 'CV1_1', 'CV1_2', 'CV1_3', 'CV1_4', 'CV1_5', 'CV1_6',
                'CV1_7', 'CV1_8']  # Declare name of the group in the partition for volume
ContactObject1 = [Contact_1, Contact_2, Contact_3, Contact_4, Contact_5, Contact_6, Contact_7, Contact_8]
Contact_name1 = ['Contact1_1', 'Contact1_2', 'Contact1_3', 'Contact1_4', 'Contact1_5', 'Contact1_6', 'Contact1_7',
                 'Contact1_8']
...
geompy.Rotate(Fuse_all_lead_encap_ROI_no_internal_face, OZ1,
              45.0 * math.pi / 180.0)  # hardwired turn because of the Lead-DBS definition (marker points and 1st contact point to Y-axis)
...
if X_2nd != Xt or Y_2nd != Yt:
    Fuse_all_lead_encap_ROI_no_internal_face = geompy.MakeRotationThreePoints(Fuse_all_lead_encap_ROI_no_internal_face,
                                                                              Vertex_O, Vertex_3, Vertex_1)

# print "Position 1st Lead at [{},{},{}], [{}',{}',{}']\n".format(Xt,Yt,Zt,OX_angle,OY_angle,OZ_angle)
for i in range(0, len(VolumeObject1)):
    geompy.TranslateDXDYDZ(VolumeObject1[i], Xt, Yt, Zt_tip)
    geompy.Rotate(VolumeObject1[i], OZ1,
                  45.0 * math.pi / 180.0)  # hardwired turn because of the Lead-DBS definition (marker points and 1st contact point to Y-axis)
...
for i in range(0, len(ContactObject1)):
    ...
    geompy.Rotate(ContactObject1[i], OZ1,
                  45.0 * math.pi / 180.0)  # hardwired turn because of the Lead-DBS definition (marker points and 1st contact point to Y-axis)
...
if (Lead2nd_Enable):
    ...
    Rest = geompy.MakeCutList(brain_solid,
                              [Fuse_all_lead_encap_ROI_no_internal_face, Fuse_all_lead_encap_ROI_no_internal_face2],
                              True)
    Partition_profile = geompy.MakePartition(VolumeObject1 + VolumeObject2 + ContactObject1 + ContactObject2, [], [],
    ...
    Group_volume = [geompy.CreateGroup(Partition_profile, geompy.ShapeType["SOLID"])] * (
                len(VolumeObject1) + len(VolumeObject2) + 1)  # +1 is Rest Group
    Group_surface = [geompy.CreateGroup(Partition_profile, geompy.ShapeType["FACE"])] * (
                len(ContactObject1) + len(ContactObject2))
else:

    Rest = geompy.MakeCutList(brain_solid, [Fuse_all_lead_encap_ROI_no_internal_face], True)
    Partition_profile = geompy.MakePartition(VolumeObject1 + ContactObject1, [], [], [], geompy.ShapeType["SOLID"], 0,
                                             [], 0)
    ###reference_volume
    ...
    Group_volume = [geompy.CreateGroup(Partition_profile, geompy.ShapeType["SOLID"])] * (
                len(VolumeObject1) + 1)  # +1 is Rest Group
...
Partition_volume_IDsList = geompy.SubShapeAllIDs(Partition_profile,
                                                 geompy.ShapeType["SOLID"])  # list all sub shape volume in Partition
...
for ref_ind in range(0, len(reference_volume)):
    ...
    for sub_ind in range(0, len(Partition_volume_IDsList)):
        ...
        if (abs(Common_volume_Pro[2] - subshape_Pro[2]) < 0.0003) and (
                abs(Common_volume_Pro[2] - reference_volume_Pro[ref_ind][2]) < 0.0003):
        ...
        elif (abs(Common_volume_Pro[2] - subshape_Pro[2]) < 0.0003) and (
                (Common_volume_Pro[2] - reference_volume_Pro[ref_ind][2]) < -0.0003):
...
if len(reference_volume) != len(Group_partition_volume):
    print("Geometry-volume error please check ROI diameter and DBS lead Position ", len(reference_volume),
          len(Group_partition_volume))
# print 'Group_partition_volume',Group_partition_volume,'\n'

### find group surface ID ######################################################################
Partition_surface_IDsList = geompy.SubShapeAllIDs(Partition_profile,
                                                  geompy.ShapeType["FACE"])  # list all sub shape face in Partition
...
for reff_ind in range(0, len(reference_surface)):
    ...
    for subf_ind in range(0, len(Partition_surface_IDsList)):
        ...
        if (abs(Common_face_Pro[1] - subshapef_Pro[1]) < 0.000001) and (
                abs(Common_face_Pro[1] - reference_surface_Pro[reff_ind][1]) < 0.000001):
        ...
        elif (abs(Common_face_Pro[1] - subshapef_Pro[1]) < 0.000001) and (
                (Common_face_Pro[1] - reference_surface_Pro[reff_ind][1]) < -0.000001):
...
if len(reference_surface) != len(Group_partition_surface):  # +len(Group_partition_Multi_surface):
    print("Geometry-Surface error please check ROI diameter and DBS lead Position ", len(reference_surface),
          len(Group_partition_surface), '\n')

# print 'Group_partition_surface',Group_partition_surface,'\n'

if (Lead2nd_Enable):
    Partition_profile = geompy.MakePartition(VolumeObject1 + VolumeObject2 + ContactObject1 + ContactObject2 + [Rest],
                                             [], [], [], geompy.ShapeType["SOLID"], 0, [], 0)
else:
    Partition_profile = geompy.MakePartition(VolumeObject1 + ContactObject1 + [Rest], [], [], [],
                                             geompy.ShapeType["SOLID"], 0, [], 0)
...
isDone = Mesh_1.SetMeshOrder([[Sub_mesh_4, Sub_mesh_3, Sub_mesh_2, Sub_mesh_1, Sub_mesh_41, Sub_mesh_42, Sub_mesh_43,
                               Sub_mesh_44, Sub_mesh_5, Sub_mesh_9, Sub_mesh_10, Sub_mesh_11, Sub_mesh_12, Sub_mesh_13,
                               Sub_mesh_14, Sub_mesh_15, Sub_mesh_16, Sub_mesh_6, Sub_mesh_7, Sub_mesh_8]])
