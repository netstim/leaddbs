...
def compute_Z_ifft(d, Xs_signal_normalized, t_vector, A,
                   i_start_octv=0.0):  # get impedance in Time (only for 2-contact systems)

    Impedance_fr_get = read_csv(os.environ['PATIENTDIR'] + '/Field_solutions/sorted_impedance.csv', delimiter=' ',
                                header=None)
    ...
    if d["spectrum_trunc_method"] == 'Octave Band Method':
        ...
        FR_vec_sign_octv = np.genfromtxt(
            os.environ['PATIENTDIR'] + '/Stim_Signal/FR_vector_signal_octaves' + str(d["trunc_param"] * 1.0) + '.csv',
            delimiter=' ')
        FR_vec_sign_octv = np.round(FR_vec_sign_octv, 6)

        Fr_corresp_ar = np.genfromtxt(
            os.environ['PATIENTDIR'] + '/Stim_Signal/Fr_corresp_array' + str(d["trunc_param"] * 1.0) + '.csv',
            delimiter=' ')
...
def convolute_and_ifft(t_steps_trunc,contact_i,last_point, Ind_trunc1, trunc_method, post_truncation, i_axon, num_segments, N_freq, phi_shift,
                       T, output):
    ...
    for i_point in range(int(num_segments)):
        ...
        Xs_Tr = np.vectorize(complex)(solution_sort[(global_i_point) * N_freq:(global_i_point * N_freq + N_freq), 3],
                                      solution_sort[global_i_point * N_freq:(global_i_point * N_freq + N_freq),
                                      4])  # real and im parts for the first point in VTA
        ...
        if global_i_point + last_point == 0:
            ...
            plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))
            plt.savefig(os.environ['PATIENTDIR'] + '/Images/1V_scaled_convoluted_1st_point.png', format='png', dpi=500)
...
def convolute_and_ifft_octaves(t_steps_trunc,contact_i, last_point, i_axon, num_segments, N_freq, N_freq_octv, phi_shift,
                               Fr_corresp_ar, Fr_octave_vec, i_start_octv, T, output):
    ...
    for i_point in range(int(num_segments)):
        ...
        Xs_Tr = np.vectorize(complex)(
            ...
            solution_sort_octv[global_i_point * N_freq_octv:(global_i_point * N_freq_octv + N_freq_octv),
            1])  # real and im parts for the first point in VTA
        ...
        for i_inx in range(Xs_Tr.shape[0]):
            if i_inx >= i_start_octv:
                ...
                stepper = stepper + 1
        if global_i_point == 0:
            np.savetxt(os.environ['PATIENTDIR'] + '/Field_solutions/Xs_Tr_full_real.csv', Xs_Tr_full_real,
                       delimiter=" ")
            np.savetxt(os.environ['PATIENTDIR'] + '/Field_solutions/Xs_Tr_full_imag.csv', Xs_Tr_full_imag,
                       delimiter=" ")
    ...
    # hf2 = h5py.File('Axons_in_time/Solution_in_time_contact_'+str(contact_i)+'.h5', 'a')
    # hf2.create_dataset('Signal_t_conv'+str(global_i_point+last_point), data = Signal_t_conv[:900])
    # hf2.close()

    output.put(i_axon)


def convolute_signal_with_field_and_compute_ifft(d, XS_signal, models_in_population, number_of_segments, FR_vec_sign,
    ...
    if d["spectrum_trunc_method"] == 'Octave Band Method':
        Fr_corresp_arr = np.genfromtxt(
            os.environ['PATIENTDIR'] + '/Stim_Signal/Fr_corresp_array' + str(d["trunc_param"] * 1.0) + '.csv',
            delimiter=' ')
        Fr_octave_vect = np.genfromtxt(
            os.environ['PATIENTDIR'] + '/Stim_Signal/Fr_octave_vector_' + str(d["trunc_param"] * 1.0) + '.csv',
            delimiter=' ')
        FR_vec_oct = np.genfromtxt(
            os.environ['PATIENTDIR'] + '/Stim_Signal/FR_vector_signal_octaves' + str(d["trunc_param"] * 1.0) + '.csv',
            delimiter=' ')
        ...
        if dif_axons == True:
            ...
            solution_over_contacts = solution_sort_octv_full[last_point * FR_vec_sign_octv.shape[0]:(
                                                                                                                last_point + models_in_population * number_of_segments) *
                                                                                                    FR_vec_sign_octv.shape[
                                                                                                        0], :]
        else:
            hf = h5py.File(name_sol[:-4] + '.h5', 'r')
    ...
    # scaling will be done later (in Axon_files)
