...
def convolute_and_ifft_octaves(last_point,i_axon,num_segments,N_freq,N_freq_octv,phi_shift,Fr_corresp_ar,Fr_octave_vec,i_start_octv,T,output):
    ...
    for i_point in range(int(num_segments)):
        global_i_point=int(num_segments*i_axon+i_point)
        Xs_Tr=np.vectorize(complex)(solution_sort_octv[(global_i_point)*N_freq_octv:(global_i_point*N_freq_octv+N_freq_octv),0],solution_sort_octv[global_i_point*N_freq_octv:(global_i_point*N_freq_octv+N_freq_octv),1])         #real and im parts for the first point in VTA
...
def convolute_signal_with_field_and_compute_ifft(S_vector,d,XS_signal,models_in_population,number_of_segments,FR_vec_sign,t_vec,A,name_sol,inx_st_oct=0,dif_axons=False,last_point=0):
    ...
    if d["spectrum_trunc_method"]=='Octave Band Method':
        ...
        if dif_axons==True:
            ...
            solution_over_contacts=solution_sort_octv_full[last_point*FR_vec_sign_octv.shape[0]:(last_point+models_in_population*number_of_segments)*FR_vec_sign_octv.shape[0],:]
        else:
            hf = h5py.File(name_sol[:-4]+'.h5', 'r')
        ...
        for point_i in np.arange(0,(models_in_population*number_of_segments)*FR_vec_sign_octv.shape[0],FR_vec_sign_octv.shape[0]):
