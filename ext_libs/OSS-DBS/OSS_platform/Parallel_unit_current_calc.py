'''
    By K. Butenko
    This script manages parallel computations in Contact_ground_calc and stores the solution
'''
...
def sort_full_solution(d,FR_vector,full_solution,number_of_points):
    ...
    if d["spectrum_trunc_method"]=='No Truncation':
        ...
        hf = h5py.File(os.environ['PATIENTDIR']+'/Field_solutions/sorted_solution_per_contact.h5', 'w')
        hf.create_dataset('dataset_1', data=solution_sort)
        hf.close()

    else:

        hf = h5py.File(os.environ['PATIENTDIR']+'/Field_solutions/sorted_solution_per_contact_'+str(d["spectrum_trunc_method"])+'_'+str(d["trunc_param"])+'.h5', 'w')
        hf.create_dataset('dataset_1', data=solution_sort)
        hf.close()
...
def calculate_in_parallel(d,freq_list,Domains,MRI_param,DTI_param,anisotropy,number_of_points,cc_multicontact):
    ...
    while i<freq_list.shape[0]:
        ...
        while j<d["number_of_processors"] and i<freq_list.shape[0]:
            ...
            processes=mp.Process(target=Contact_ground_calc.compute_fields_from_unit_currents,args=(Sim_setup,Solver_type,Vertices,Domains,j,Field_on_VTA,output))
