        ...
        if contains(tractName,'_lh')  || contains(tractName,'_rh') || contains(tractName,'_left')  || contains(tractName,'_right')
            if (side == 2 && (contains(tractName,'_lh')  || contains(tractName,'_left'))) || (side == 1 && (contains(tractName,'_rh')  || contains(tractName,'_right')))
                ...
                if strcmp(targetType(t),"SSE")
                    jsonDict.Soft_SE_dict.(genvarname(['SSE', side_suffix])).(genvarname(tractName)) = [targetActivation(t), 1.0, pathwayWeights(t)];  % [target_rate, mean_val (not relevant), sum_val (can be used to emphasize an importance of the pathway)]
                elseif strcmp(targetType(t),"CSE") == 1
                    jsonDict.SE_dict.(genvarname(['CSE', side_suffix])).(genvarname(tractName)) = [targetActivation(t), 1.0, pathwayWeights(t)];
                else
