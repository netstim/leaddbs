function ea_fsl_gencoordwarpfile(warpResolution,warpCoefFilename, inversewarpfile, hdr)

if ~exist(inversewarpfile,'file')
    ...
    command =  sprintf(' --in=%s --ref=%s --out=%s  --withaff', warpCoefFilename, inversewarpfile, inversewarpfile);
    if verbose
