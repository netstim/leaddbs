...
fnirtstage = [' --ref=',fixedimage{1},' --in=',movingimage{1},' --aff=',directory,'aff_anat2mni.mat --inmask=',ea_path_helper(mask),' --refmask=',ea_path_helper([ea_getearoot,'templates',filesep,'bmni_hires.nii.gz']),' --iout=',outputimage,' --cout=',directory,glbase,'Composite.nii --verbose --subsamp=4,2 --warpres=8,8 --miter=5,10 --infwhm=8,2 --reffwhm=4,0 --lambda=300,30 --estint=1,0 --ssqlambda=1 --regmod=bending_energy --intmod=global_non_linear_with_bias --intorder=5 --biasres=50,50,50 --biaslambda=10000 --refderiv=0 --applyrefmask=1,1 --applyinmask=1,1'];

invwarpstage = [' -w ',directory,glbase,'Composite.nii -o ',directory,glbase,'InverseComposite.nii.gz -r ',movingimage{1}];
