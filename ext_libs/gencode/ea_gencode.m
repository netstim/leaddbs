...
%
% See also ea_gencode_RVALUE, ea_gencode_SUBSTRUCT, ea_gencode_SUBSTRUCTCODE.
%
...
[rstr sts] = ea_gencode_rvalue(item);
if sts
    ...
    switch class(item)
        ...
        case 'struct'
            str = ea_gencode_structobj(item, tag, tagctx);
        otherwise
...
function str = ea_gencode_structobj(item, tag, tagctx)
...
elseif numel(item) == 1
    ...
    for k = 1:numel(subs)
        if ~isstruct(item)
            str{end+1} = sprintf('%s = %s;', ea_gencode_substruct(subs{k}, tag), citem);
        end
        for l = 1:numel(fn)
