...
if isempty(hmain),
    ...
    uicontrol('Style','checkbox','String','auto','Position',pos + [180 0 20 0],'Value',false,'HorizontalAlignment','left','tag','fiberGT_autosave');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    pos = [15 490 100 25];
...
%
      ...
      elseif strcmp(varargin{k},'reactivate');
            ready;
      elseif strcmp(varargin{k},'doAccum');
            h = findobj('tag','fiberGT_accum');
            oldv = get(h,'value');
            set(h,'value',true);
            startstop_Callback(gcf,[],[]);
            set(h,'value',oldv);
...
%       fprintf('precomputing approx. model (#dirs=%i, alpha=%f, order=%i)\n',size(bTensor,3),alpha,ordermax(1));
...
function mexpcRJMCMC
       ...
       [pathtofgt dummy] = fileparts(mfilename('fullpath'));
       mexstr = sprintf('mex CXXFLAGS="\\$CXXFLAGS %s" %s -outdir %s  %s',compilerparams,fullfile(pathtofgt,'ccode','pcRJMCMC.cpp') , fullfile(pathtofgt,'ccode'),libs);
       fprintf('%s\n',mexstr);
       eval(mexstr);
...
function preprocessData(bTensor,signal,mask)
       ...
       b0idx = find(bval <= 0.1);
       datastruct.b0avg = single(mean(single(signal(:,:,:,b0idx)),4));
       datastruct.stderr = single(std(single(signal(:,:,:,b0idx)),[],4));
       ...
       signal = permute(single(signal),[4 1 2 3]);
       signal = signal(:,mask(:));
...
%        model2alphaLUT = single( model2alphaLUT);
       ...
       if not(Pstruc.nonshell),
            bvalu = unique(round(bval*10));
            nmax = length(bvalu)-1;
            for k = 2:length(bvalu),
                radialFun{k-1} = double(round(bval*10) == bvalu(k));
            end;
       else
           nmax = Pstruc.nmax;
           for k = 1:nmax,
               radialFun{k} = Pstruc.bfun(bval,k);
           end;
       end;


       datastruct.numbvals = nmax;
       model2alphaLUT = zeros([size(dirs,2) size(bTensor,3) lmax/2-1 nmax]);
       M2 = zeros(7,size(bTensor,3),nmax);


       for k = 1:nmax,
           d = bDir;
           C = dirs'*d ;
           leg = reshape(myleg(lmax,C(:)),[size(C) lmax+1]);
           leg = leg(:,:,5:2:end);

           radFun = radialFun{k};

           leg = leg.*repmat(radFun,[size(leg,1) 1 size(leg,3)]);
           model2alphaLUT(:,:,:,k) = leg/sum(radFun) ;

           q = d;
           M =  [ 1+0*q(1,:) ; ...
                 -(-2*q(1,:).^2+q(2,:).^2+q(3,:).^2)/sqrt(2*9) ; -(-2*q(2,:).^2+q(1,:).^2+q(3,:).^2)/sqrt(2*9) ; -(-2*q(3,:).^2+q(1,:).^2+q(2,:).^2)/sqrt(2*9) ...
                ; q(1,:).*q(2,:) ; q(1,:).*q(3,:) ; q(3,:).*q(2,:) ];
           M2(:,:,k) = M/sum(radFun) .* repmat(radFun,[size(M2,1) 1 ]) ;
...
%            leg = leg(:,:,5:2:end);
%
%            model2alphaLUT(:,idx,:,k-1) = leg/sum(idx);
%
%            q = d;
%            M =  [ 1+0*q(1,:) ; ...
%                  -(-2*q(1,:).^2+q(2,:).^2+q(3,:).^2)/sqrt(2*9) ; -(-2*q(2,:).^2+q(1,:).^2+q(3,:).^2)/sqrt(2*9) ; -(-2*q(3,:).^2+q(1,:).^2+q(2,:).^2)/sqrt(2*9) ...
%                 ; q(1,:).*q(2,:) ; q(1,:).*q(3,:) ; q(3,:).*q(2,:) ];
%            M2(:,idx,k-1) = M/sum(idx);
%
%        end;
       model2alphaLUT = reshape(model2alphaLUT,[size(model2alphaLUT,1) size(model2alphaLUT,2) size(model2alphaLUT,3)*size(model2alphaLUT,4)]);
       model2alphaLUT = permute(model2alphaLUT,[3 1 2]);





       model2alphaLUT = reshape(model2alphaLUT,[size(model2alphaLUT,1)*size(model2alphaLUT,2) size(model2alphaLUT,3)]);

       M2 = permute(M2,[1 3 2]); M2 = reshape(M2,[size(M2,1)*size(M2,2) size(M2,3)]);

       model2alphaLUT = cat(1,model2alphaLUT,M2);
       datastruct.signal = model2alphaLUT* signal(:,:);
       datastruct.bvals = bval;%u/bdiscstep;
...
function varargout = saveftr_Callback(h,eventdata,handles,varargin)
    ...
    [ftr lens] = saveFTR(ftrname,datastruct,fibidx,datastruct.vox,info,true);
...
function saveFD(sel,M)
    ...
    ftr = saveFTR(ftrname,datastruct,fibidx,datastruct.vox,info,false);
...
function datastruct = trackit(datastruct,Tstart,Tend,ni,its,info)
    ...
    ftrname = getftrnameFromGUI;
    accum_ftrname = strrep(ftrname,'_FTR','_accum_FTR');
    accum_numits = datastruct.params.accum_numits;
    accum_numavg = datastruct.params.accum_numavg;
    ...
    [pathtofgt dummy] = fileparts(mfilename('fullpath'));
    thedir = fullfile(pathtofgt,'ccode');
    ...
    while(1),
          ...
          paramStruct.num_qspace_samples = 120;
          paramStruct.GM_mask = double(size(datastruct.spatialProbabilities,4)>1);
          paramStruct.numbvals = datastruct.numbvals;


          if doaccum,
               paramStruct.lam_DE = 0;
               paramStruct.maxit = accum_numits;
               T = datastruct.params.accum_Temp;
          end;
         ...
         % xx = dir([thedir '/pcRJMCMC.*']); fprintf('executable last change: %s\n',xx.date);
          ...
          eval(sprintf('[datastruct.state datastruct.vfmap stat clist segparams] = %s(datastruct.state,datastruct.vfmap,datastruct.signal,mask,datastruct.S2,double(vox),paramStruct,datastruct.sphereInterpolation,h);',mexfilename));
          cd(curdir);
          neededtime = toc;
          ...
          datastruct.segparams=segparams;
          fibidx = BuildFibres(datastruct.state,fiblen);
          ...
          autosave = get(findobj('tag','fiberGT_autosave'),'value');
          if ck == ni,
              autosave = true;
          end;
          ...
          [ftr lens] = saveFTR(ftrname,datastruct,fibidx,datastruct.vox,info,autosave);
         ...
         if stopButtonPressed,
          ...
          if doaccum,
              accumftr = mergeFTR(ftr,accumftr);
              if cnt > accum_numavg,
                  break;
              end;
          end
    ...
    if doaccum,
        ftrstruct_write(accumftr,accum_ftrname);
    end;
