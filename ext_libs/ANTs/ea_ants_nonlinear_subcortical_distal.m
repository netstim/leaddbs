...
synmask2stage = [' --transform SyN[0.3]'...
    ...
    ' --shrink-factors ', affineshrinkfactors ...
    ' --smoothing-sigmas ', affinesoomthingssigmas, ...
    ' --masks [',ea_space([],'subcortical'),'secondstepmask.nii',',nan]'];
synmask3stage = [' --transform SyN[0.3]'...
    ' --convergence ', affineconvergence, ...
    ' --shrink-factors ', affineshrinkfactors ...
    ' --smoothing-sigmas ', affinesoomthingssigmas, ...
    ' --masks [',ea_space([],'subcortical'),'thirdstepmask.nii',',nan]'];
synmask4stage = [' --transform SyN[0.3]'...
    ' --convergence ', affineconvergence, ...
    ' --shrink-factors ', affineshrinkfactors ...
    ' --smoothing-sigmas ', affinesoomthingssigmas, ...
    ' --masks [',ea_space([],'subcortical'),'fourthstepmask.nii',',nan]'];
...
for fi=1:length(fixedimage)
    ...
    synstage=[synstage,...
        ' --metric ',metrics{fi},'[', fixedimage{fi}, ',', movingimage{fi}, ',',num2str(weights(fi)),suffx,']'];
    synmask2stage=[synmask3stage,...
        ' --metric ',metrics{fi},'[', fixedimage{fi}, ',', movingimage{fi}, ',',num2str(weights(fi)),suffx,']'];
    synmask3stage=[synmask3stage,...
        ' --metric ',metrics{fi},'[', fixedimage{fi}, ',', movingimage{fi}, ',',num2str(weights(fi)),suffx,']'];
    synmask4stage=[synmask4stage,...
        ' --metric ',metrics{fi},'[', fixedimage{fi}, ',', movingimage{fi}, ',',num2str(weights(fi)),suffx,']'];
end
...
cmd = [ANTS, ' --verbose 1' ...
             ...
             rigidstage, affinestage, synstage, synmask2stage, synmask3stage, synmask4stage];
