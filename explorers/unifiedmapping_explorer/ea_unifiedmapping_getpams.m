function [pamlist,FilesExist] = ea_unifiedmapping_getpams(obj)
        ...
        for side = 1:2
            ...
            if all(S.amplitude{1,side} == 0)
                ...
                if side == 1
                    BIDS_side = 'R';
                    % mirrored
                    pamlist{sub+numPatient,1} = fullfile(stimFolder,[obj.calcsettings.fibfilt_connectome, filesep, 'PAM', filesep, subSimPrefix, 'fiberActivation_model-ossdbs_hemi-L.mat']);
                else
                    BIDS_side = 'L';
                    % mirrored
                    pamlist{sub+numPatient,2} = fullfile(stimFolder,[obj.calcsettings.fibfilt_connectome, filesep, 'PAM', filesep, subSimPrefix, 'fiberActivation_model-ossdbs_hemi-R.mat']);
                end
                pamlist{sub,side} = fullfile(stimFolder,[obj.calcsettings.fibfilt_connectome, filesep, 'PAM', filesep, subSimPrefix, 'fiberActivation_model-ossdbs_hemi-',BIDS_side,'.mat']);
