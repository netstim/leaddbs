function ea_unified_get_Eproj(explorer,vatlist)
...
if explorer.calcsettings.multi_pathways == 1
    % check if merged_pathways is in fibfiltering folder
    [filepath,~,~] = fileparts(explorer.analysispath);
    merged_connectome = [filepath,filesep,explorer.calcsettings.fibfilt_connectome,filesep,'merged_pathways.mat'];
    ...
    merged_connectome = [ea_getconnectomebase('dMRI'), explorer.calcsettings.fibfilt_connectome, filesep, 'data.mat'];
end

% find which space and stim folder is used
if ~isfield(explorer.M,'pseudoM')
    stim_space = [ea_unifiedmapping_nt(explorer.calcsettings.calcspace),'gs_',explorer.M.guid]; 
end
        ...
        elseif isfile(vatlist{pt_i,side})
            ...
            if isfield(explorer.M,'pseudoM')
                if explorer.M.pseudoM
                    ...
                    stim_space = [ea_unifiedmapping_nt(explorer.calcsettings.calcspace),stim_folder];
                end
            end  
