function varargout = ea_sweetspotcontrol_hemi(varargin)
% EA_SWEETSPOTCONTROL_HEMI MATLAB code for ea_sweetspotcontrol_hemi.fig
%      EA_SWEETSPOTCONTROL_HEMI, by itself, creates a new EA_SWEETSPOTCONTROL_HEMI or raises the existing
%      singleton*.
%
%      H = EA_SWEETSPOTCONTROL_HEMI returns the handle to a new EA_SWEETSPOTCONTROL_HEMI or the handle to
%      the existing singleton*.
%
%      EA_SWEETSPOTCONTROL_HEMI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EA_SWEETSPOTCONTROL_HEMI.M with the given input arguments.
%
%      EA_SWEETSPOTCONTROL_HEMI('Property','Value',...) creates a new EA_SWEETSPOTCONTROL_HEMI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ea_sweetspotcontrol_hemi_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ea_sweetspotcontrol_hemi_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ea_sweetspotcontrol_hemi

% Last Modified by GUIDE v2.5 26-Feb-2018 12:27:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ea_sweetspotcontrol_hemi_OpeningFcn, ...
                   'gui_OutputFcn',  @ea_sweetspotcontrol_hemi_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ea_sweetspotcontrol_hemi is made visible.
function ea_sweetspotcontrol_hemi_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ea_sweetspotcontrol_hemi (see VARARGIN)

% Choose default command line output for ea_sweetspotcontrol_hemi
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ea_sweetspotcontrol_hemi wait for user response (see UIRESUME)
...
% factors
factorLabels = [handles.factorLabel1_L, handles.factorLabel1_R
                handles.factorLabel2_L, handles.factorLabel2_R
                handles.factorLabel3_L, handles.factorLabel3_R
                handles.factorLabel4_L, handles.factorLabel4_R];
for side=1:2
    ...
    switch side
        case 1
            sliderName = 'L';
            sliderTooltip = 'Left hemisphere';
        case 2
            sliderName = 'R';
            sliderTooltip = 'Right hemisphere';
    end
    for f=size(obj.nii.X,1):-1:1
        set(0,'CurrentFigure',handles.roicontrol);
        jSlider{cnt} = javax.swing.JSlider(0,100);
        factorLabelPos = get(factorLabels(f,side), 'Position');
        ...
        set(jSlider{cnt}, 'Value', 100/length(obj.nii.factors),...
            'Background',java.awt.Color(1,1,1),...
            'MajorTickSpacing',0.1, 'PaintLabels',true, ... % with labels, no ticks
            'Name', sliderName, 'ToolTipText', sliderTooltip);

        FactorSlider{f,side} = handle(jSlider{cnt}, 'CallbackProperties');
        set(FactorSlider{f,side}, 'MouseReleasedCallback', {@sliderFactorChange,obj,handles,f});  %alternative
        set(FactorSlider{f,side}, 'StateChangedCallback', {@sliderFactorChangeLive,obj,handles,f});  %alternative

        set(factorLabels(f,side), 'Visible', 'on', 'HorizontalAlignment', 'left', 'String',strrep(obj.nii.factors{f}, '_', ' '));
...
ofactors=1:length(obj.nii.factors);
ofactors(factor)=[];
...
for osliders=ofactors
    FactorSlider{osliders, side}.Value=obj.fmix(osliders)*100;
end
...
for slider=1:length(FactorSlider)
    tval(slider)=FactorSlider{slider, side}.Value;
...
% scale remaining sliders accordingly:
for osliders=ofactors
    FactorSlider{osliders, side}.Value=tval(osliders)*100;
end


% --- Outputs from this function are returned to the command line.
function varargout = ea_sweetspotcontrol_hemi_OutputFcn(hObject, eventdata, handles)
