...
cmdstring=['pbsubmit -q highio -O ',[options.root,options.patientname,filesep,'job_',jobID],'.out -E ',[options.root,options.patientname,filesep,'job_',jobID],'.err -c "matlab.new -singleCompThread -nodisplay -r "''ea_run runcluster ',[options.root,options.patientname,filesep,'job_',jobID],'''"'];
keyboard
system(cmdstring);
