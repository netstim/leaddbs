function electrode=ea_elspec_aleva_directstim_11500(varargin)
% Generate electrode model of Aleva directSTIM 11500

options.elmodel='Aleva directSTIM 11500';
...
trajectory=[zeros(lengthration*rescaleratio,2),linspace(lengthration*rescaleratio,0,lengthration*rescaleratio)'];
trajvector=mean(diff(trajectory));
trajvector=trajvector/norm(trajvector);

startpoint=trajectory(1,:)-(1.5*(coords_mm(1,:)-trajectory(1,:)));
...
for cntct=1:elspec.numel-1
    ...
    htd=(max(cZ(:))/2);
    cZ=cZ-htd;
    hait=coords_mm(cntct,3)+elspec.contact_length/2+elspec.contact_spacing/2;
    cZ=cZ+hait;
...
X1=coords_mm(1,:)+trajvector*(elspec.contact_length/2);
...
if any(axis_rot) || angle_X1X2
    % rotate(elrender(cnt),axis_rot,angle_X1X2,X1)
end
ea_specsurf(elrender(cnt),usecolor,aData);
...
ea_genvol_aleva(meshel,elspec,rescaleratio,vizz);
...
if vizz
    ...
    for ins=1:length(electrode.insulation)
        ...
        elrender(cnt)=patch('Faces',electrode.insulation(ins).faces,'Vertices',electrode.insulation(ins).vertices);
        if isfield(elstruct,'group')
            usecolor=elstruct.groupcolors(elstruct.group,:);
        else
            usecolor=elspec.lead_color;
    ...
    for con=1:length(electrode.contacts)
        ...
        elrender(cnt)=patch('Faces',electrode.contacts(con).faces,'Vertices',electrode.contacts(con).vertices);

        ea_specsurf(elrender(cnt),elspec.contact_color,aData);
