function electrode=ea_elspec_stjudeactivetip(varargin)
...
if nargin
    ...
    options.elmodel='St. Jude ActiveTip';
end
...
for comp=1:options.elspec.numel*2
    ...
    try % shaft and contacts, here three surface components
        ...
        if cnt==4 % combine comp 4,5,6 + 25 (=tip) to first contact
            tip=elrender{side}(25);
            tip=surf2patch(tip,'triangles');
            cyl.faces=[cyl.faces;tip.Faces+length(cyl.vertices)];
            cyl.vertices=[cyl.vertices;tip.Vertices];
        end        
        cnt=cnt+3;
...
save([fileparts(which('lead')),filesep,'templates',filesep,'electrode_models',filesep,'stjudeactivetip'],'electrode');
