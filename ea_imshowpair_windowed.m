function  ea_imshowpair_windowed( Img, options ,addstring)
...
MainImage = 1;
try % image toolbox
    imshow(squeeze(Img(:,:,S,MainImage)), [Rmin Rmax])
catch    
    imagesc(squeeze(Img(:,:,S,MainImage)), [Rmin Rmax])
end
...
%            set(stxthand, 'String', sprintf('Slice# %d / %d',S, sno));
 ...
 %           set(stxthand, 'String', '2D image');
        ...
        set(get(gca,'children'),'cdata',Img(:,:,S,MainImage))
    end
...
%             set(gcf, 'WindowButtonMotionFcn', @WinLevAdj);
           ...
           set(a2, ...
              ...
              'Color',get(a1,'Color'), ...
              'Box','on');
          set(i2,'CData',Img(:,:,S,1+mod(MainImage,2)));
...
% -=< Axial view callback function >=-
    function AxialView(object,eventdata)
        ...
        cla(hdl_im);
        hdl_im = axes('position',[0,0,1,1]);
        imshow(Img(:,:,S,MainImage), [Rmin Rmax])
