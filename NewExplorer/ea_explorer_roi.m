classdef ea_explorer_roi < handle
    ...
    methods
        ...
        function obj=ea_explorer_roi(niftiFilename,pobj) % generator function
            if nargin
                ...
                if isfield(pobj, 'colorscale')
                    obj.colorscale = pobj.colorscale;
                end
        ...
        function obj=update_roi(obj,evtnm) % update ROI
            ...
            if ismember(evtnm,{'all','threshold','smooth','hullsimplify','usesolidcolor'}) % need to recalc fv here:
                ...
                %% lets stay in voxel space until the end - makes stuff alot easier
                img = permute(obj.nii.img,[2,1,3]);
                img(isnan(img)) = 0;
                fv = isosurface(img,obj.threshold);
                fv2 = isocaps(img,obj.threshold);
                fv.vertices = vertcat(fv.vertices,fv2.vertices);
                fv.faces = vertcat(fv.faces,fv2.faces);                
                obj.fv = fv;                
                clear fv fv2 img
                %% smoothing and other stuff                
                if obj.smooth
                ...
                if obj.binary || obj.usesolidcolor
                     ...
                     if isempty(obj.colorscale)
                         obj.colorscale = [min(obj.nii.img,[],'all','omitnan'),max(obj.nii.img,[],'all','omitnan')];
                     end                     
                     mycols = ea_explorer_isocolors(obj.nii.img,obj.sfv.vertices);                
                     obj.cdat = squeeze(ea_explorer_value2color(mycols,obj.colormap,obj.colorscale));
                end                
                %% bring to mm space
                obj.sfv.vertices = obj.nii.mat * [obj.sfv.vertices'; ones(1,length(obj.sfv.vertices(:,1)))];
                obj.sfv.vertices = obj.sfv.vertices(1:3,:)';                
            end
            ...
            if ~isempty(obj.plotFigureH)
                ...
                if obj.binary || obj.usesolidcolor
                    ...
                    set(obj.patchH,...
                        {'FaceVertexCData','FaceColor','CDataMapping'},...
                        {obj.cdat,'flat','direct'});
                end
            end
