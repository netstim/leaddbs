classdef ea_explorerclass < handle
    ...
    methods
        ...
        function initialize(obj,datapath,resultfig)
            ...
            % Visualization
            obj.visualization.roivisible = 0; % show ROI (usually VTAs)
            obj.visualization.connfibvisible = 0;% show all connected tracts in white
            obj.visualization.posvisible = 1; % pos tract visible
            obj.visualization.negvisible = 0; % neg tract visible
            obj.visualization.showposamount = [25 25]; % two entries for right and left
            obj.visualization.shownegamount = [25 25]; % two entries for right and left
            obj.visualization.poscolor = [0.9176,0.2000,0.1373]; % positive main color
            obj.visualization.negcolor = [0.2824,0.6157,0.9725]; % negative main color
            obj.visualization.showsignificantonly = 0;
            % Cleartune
            obj.cleartune.results = [];% copy of results for auto tuning functions
            obj.cleartune.efields = [];% efields used to calc results
            obj.cleartune.injected = [];% status to report file has injected values
            obj.cleartune.optim = 0;
            obj.cleartune.vars = [];
            % Multipath
        ...
        function draw(obj,vals,fibcell,usedidx) %for cv live visualize
            ...
            elseif ~exist('vals','var')
                [vals,fibcell,usedidx]=ea_explorer_calcstats(obj);
            end
            ...
            if obj.visualization.connfibvisible
                obj.runwhite=1;
                [~,cfibcell,~]=ea_explorer_calcstats(obj);
                obj.runwhite=0;
            ...
            if obj.multipath.multi_pathways == 1 && (isequal(ea_method2methodid(obj),'VAT_Ttest') || isequal(ea_method2methodid(obj),'PAM_Ttest') || isequal(ea_method2methodid(obj),'plainconn'))% at the moment, obj.connFiberInd is defined only for OSS-DBS
                ...
                for side = 1:size(usedidx,2)
                    num_per_path{side} = zeros(1,length(obj.multipath.map_list));
                    if length(usedidx{side})
                        for inx = 1:length(usedidx{side})
                            ...
                            if obj.connectivity_type == 2
                                [d, ix] = min(abs(obj.multipath.map_list-obj.results.(ea_conn2connid(obj.connectome)).connFiberInd_PAM{side}(usedidx{side}(inx))));
                                if (obj.multipath.map_list(ix)-obj.results.(ea_conn2connid(obj.connectome)).connFiberInd_PAM{side}(usedidx{side}(inx))) > 0
                                    ix = ix - 1;
                                end
                            else
                                [d, ix] = min(abs(obj.multipath.map_list-obj.results.(ea_conn2connid(obj.connectome)).connFiberInd_VAT{side}(usedidx{side}(inx))));
                                if (obj.multipath.map_list(ix)-obj.results.(ea_conn2connid(obj.connectome)).connFiberInd_VAT{side}(usedidx{side}(inx))) > 0
                                    ix = ix - 1;
                                end
                            end
                ...
                if ~isempty(num_per_path{1})
                    ...
                    title('Right HS')
                    % Create legend
                    lgd = legend(obj.multipath.pathway_list(nonZero_idx));
                ...
                if ~isempty(num_per_path{2})
                    ...
                    title('Left HS')
                    % Create legend
                    lgd2 = legend(obj.multipath.pathway_list(nonZero_idx));
            ...
            if strcmp(obj.multitractmode,'Single Tract Analysis') || obj.subscore.special_case
                % reset colorbar
                obj.colorbar=[];
                if ~any([obj.visualization.posvisible,obj.visualization.negvisible])
                    return
                end
            end
            ...
            for group=1:size(vals,1) % vals will have 1x2 in case of bipolar drawing and Nx2 in case of group-based drawings (where only positives are shown).
                ...
                if strcmp(obj.multitractmode,'Split & Color By Subscore') || strcmp(obj.multitractmode,'Split & Color By PCA')
                    if obj.subscore.special_case
                        ...
                        if group == 1
                            ...
                            if obj.visualization.posvisible && all(vals_across_subscores<0)
                                obj.visualization.posvisible = 0;
                                fprintf('\n')
                            ...
                            if obj.visualization.negvisible && all(vals_across_subscores>0)
                                obj.visualization.negvisible = 0;
                                fprintf('\n')
                                warning('off', 'backtrace');
                    ...
                    if obj.visualization.posvisible && all(allvals<0)
                        obj.visualization.posvisible = 0;
                        fprintf('\n')
                        warning('off', 'backtrace');
                        warning('No positive values found, posvisible is set to 0 now!');
                    ...
                    if obj.visualization.negvisible && all(allvals>0)
                        obj.visualization.negvisible = 0;
                        fprintf('\n')
                        warning('off', 'backtrace');
                        warning('No negative values found, negvisible is set to 0 now!');
                ...
                if domultitract % also means subscores
                    switch obj.multitractmode
                        ...
                        %two.
                        case 'Split & Color By Group'
                            obj.visualization.poscolor = linecols(group,:);
                            obj.visualization.negcolor = [0.94,0.97,1.00];
                            if obj.subscore.special_case
                                cmap = ea_colorgradient(gradientLevel, [1,1,1], obj.visualization.poscolor);
                                if obj.visualization.posvisible && ~obj.visualization.negvisible
                                    fibcmap{group} = ea_colorgradient(gradientLevel, cmap(shiftedCmapStart,:), obj.visualization.poscolor);
                                    cmapind = round(normalize(allvals,'range',[1,gradientLevel]));
                                    alphaind = ones(size(allvals));
                                    % alphaind = normalize(allvals, 'range');
                                elseif ~obj.visualization.posvisible && obj.visualization.negvisible
                                    cmap = ea_colorgradient(gradientLevel, obj.visualization.negcolor, [1,1,1]);
                                    fibcmap{group} = ea_colorgradient(gradientLevel, obj.visualization.negcolor, cmap(shiftedCmapEnd,:));
                                    cmapind = round(normalize(allvals,'range',[1,gradientLevel]));
                                    alphaind = ones(size(allvals));
                                    % alphaind = normalize(-allvals, 'range');
                                else
                                ...
                                if obj.subscore.posvisible(group) && obj.subscore.negvisible(group)
                                    cmap = ea_colorgradient(gradientLevel/2, obj.visualization.negcolor, [1,1,1]);
                                    cmapLeft = ea_colorgradient(gradientLevel/2, obj.visualization.negcolor, cmap(shiftedCmapLeftEnd,:));
                                    cmap = ea_colorgradient(gradientLevel/2, [1,1,1], obj.visualization.poscolor);
                                    cmapRight = ea_colorgradient(gradientLevel/2, cmap(shiftedCmapRightStart,:), obj.visualization.poscolor);
                        ...
                        otherwise
                            if  strcmp(obj.multitractmode,'Split & Color By Subscore')
                                obj.visualization.poscolor = obj.subscore.colors{1,2}(group,:); % positive main color
                                obj.visualization.negcolor = obj.subscore.colors{1,1}(group,:); % negative main color
                            elseif strcmp(obj.multitractmode,'Split & Color By PCA')
                                obj.visualization.poscolor = obj.subscore.pcacolors(group,:);
                                obj.visualization.negcolor = [0.94,0.97,1.00];
                            end
                            if obj.subscore.special_case %operating the mixed fiber tract in the multitract mode. Essentially uses the same logic as you would have used if you were not doing multitract mode, but it incorporates the multitract analysis.
                                ...
                                if obj.visualization.posvisible && obj.visualization.negvisible
                                    ...
                                    % alphaind(allvals<0) = normalize(-1./(1+exp(-allvals(allvals<0))), 'range');
                                    % alphaind(allvals>0) = normalize(1./(1+exp(-allvals(allvals>0))), 'range');
                                elseif obj.visualization.posvisible
                                    cmap = ea_colorgradient(gradientLevel, [1,1,1], obj.visualization.poscolor);
                                    fibcmap{group} = ea_colorgradient(gradientLevel, cmap(shiftedCmapStart,:), obj.visualization.poscolor);
                                    cmapind = round(normalize(allvals,'range',[1,gradientLevel]));
                                    alphaind = ones(size(allvals));
                                    % alphaind = normalize(1./(1+exp(-allvals)), 'range');
                                elseif obj.visualization.negvisible
                                    cmap = ea_colorgradient(gradientLevel, obj.visualization.negcolor, [1,1,1]);
                                    fibcmap{group} = ea_colorgradient(gradientLevel, obj.visualization.negcolor, cmap(shiftedCmapEnd,:));
                                    cmapind = round(normalize(allvals,'range',[1,gradientLevel]));
                                    alphaind = ones(size(allvals));
                    ...
                    obj.visualization.poscolor = obj.visualization.poscolor; % positive main color
                    obj.visualization.negcolor = obj.visualization.negcolor; % negative main color
