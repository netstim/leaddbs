...
for side=1:numel(vals)
    ...
    obj.stats.pos.shown(side)=sum(vals{1,side}>0);
    obj.stats.neg.shown(side)=sum(vals{1,side}<0);
    ...
    if size(vals,2)>1 % standard case
        cmapind = mat2cell(cmapind, [numel(vals{1,1}), numel(vals{1,2})])';
        alphaind = mat2cell(alphaind, [numel(vals{1,1}), numel(vals{1,2})])';
    else % potential scripting case, only one side
        cmapind = mat2cell(cmapind, numel(vals{1,1}))';
        alphaind = mat2cell(alphaind, numel(vals{1,1}))';
    end
