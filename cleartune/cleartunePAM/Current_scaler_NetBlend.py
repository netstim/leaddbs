...
def compute_global_score(S_vector, *args):
    ...
    # first check strict side-effects
    for key in SE_dict:
    ...
    with open(d['symptom_weights_file'], 'r') as fp:
        symptom_weights = json.load(d['symptom_weights_file'])
    ...
    # for now it is a simple summation (exactly like in Optim_strategies.py)
    ...
    # import pandas
    # PD_profiles = pandas.read_csv('PD_symptom_profiles.csv')
    # PD_SE_thresh = pandas.read_csv(
    #     'PD_SE_thresholds.csv')  # strict thresholds for side-effects that constrain the optimization
    #
    # profile_weighting = np.genfromtxt('PD_profiles_weighting.csv', dtype=float, delimiter=',', names=True)
    #
    # global_score = 0.0
    #
    # # first check the strict thresholds
    # for SE_profile in PD_SE_thresh:
    #     for i in range(len(activation_profile)):
    #         if activation_profile[i] > PD_SE_thresh[SE_profile].values[i]:
    #             return 1000000.0
    #
    # k = 0
    # for symp_profile in PD_profiles:
    #     if len(PD_profiles[symp_profile].values) == 1:
    #         logging.critical("Computed activation rate - Optimal rate for {}: {}".format(S_vector, activation_profile - PD_profiles[symp_profile].values[0]))
    #         distance = abs(activation_profile - PD_profiles[symp_profile].values[0])
    #     else:
    #         from scipy.spatial.distance import canberra, cityblock, euclidean, braycurtis, cosine
    #         if d['similarity_metric'] == 'Canberra':
    #             distance = canberra(activation_profile, PD_profiles[symp_profile].values, w = profile_weighting[k])
    #         elif d['similarity_metric'] == 'Manhattan':
    #             distance = cityblock(activation_profile, PD_profiles[symp_profile].values, w = profile_weighting[k])
    #         elif d['similarity_metric'] == 'Euclidean':
    #             distance = euclidean(activation_profile, PD_profiles[symp_profile].values, w = profile_weighting[k])
    #         elif d['similarity_metric'] == 'Cosine':
    #             distance = cosine(activation_profile, PD_profiles[symp_profile].values, w = profile_weighting[k])
    #         elif d['similarity_metric'] == 'Bray-Curtis':
    #             distance = braycurtis(activation_profile, PD_profiles[symp_profile].values, w = profile_weighting[k])
    #         else:
    ...
    #
    #     global_score += d['symptom_weighting'][k] * distance
    #     k += 1
