...
if ischar(options) % return name of method.
    varargout{1}='Edgedetection, use ML imagetoolbox';
...
S(:)=0;
Y=affine(X,mat,1,0);
S(1:lsize(X,S,1),1:lsize(X,S,2),1:lsize(X,S,3))=Y(1:lsize(X,S,1),1:lsize(X,S,2),1:lsize(X,S,3));

function sz=lsize(X,S,dim)
if size(X,dim)<size(S,dim)
    sz=size(X,dim);
else
    sz=size(S,dim);
end
