...
if ~exist([options.root,options.patientname,filesep,'bgmsk.nii'],'file')
    ...
    if exist([options.root,options.patientname,filesep,'glanat.nii'],'file') % if normalization has been done already, use inverse warp to map bgmask to anat_t1.nii
        ea_apply_normalization_tofile(options,{[options.root,options.patientname,filesep,'bgmsk.nii']},{[options.root,options.patientname,filesep,'bgmsk.nii']},[options.root,options.patientname,filesep],1,0);
