function [goodz]=sample_cuboid(diams,fitline,trajvector,patientname,zdist,options)
...
switch options.eldist
    case 2
        headtemp=load_nii(fullfile(pwd,'templates','electrode_contacts','2mm.nii'));
    case 3
        headtemp=load_nii(fullfile(pwd,'templates','electrode_contacts','3mm.nii'));        
end
...
for xx=1:size(imat,1)
    for yy=1:size(imat,2)
        for lag=min(fitline(:,3)):size(imat,3)-size(htemp,3)
            corrs(cnt,lag+1-min(fitline(:,3)))=corr(squeeze(imat(xx,yy,lag:lag+size(htemp,3)-1)),squeeze(htemp(xx,yy,:)),'rows','pairwise');
...
[maxval,maxind]=max(corrs);

goodz=maxind+min(fitline(:,3))+5; % -1 because lag-series starts with 0 and not one. +5 because templates start 5 voxels before electrode point.
showdis(['Maximal value was: ',num2str(maxval),'.'],options.verbose);
catch
   showdis(['Probably algorithm stopped to early. No guess of electrode heights possible.'],options.verbose);
   goodz=min(fitline(:,3));
end
