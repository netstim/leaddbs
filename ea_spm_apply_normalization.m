function ea_spm_apply_normalization(options)
% Apply SPM based normalization to pre-op anchor and post-op images

% Use INVERSE deformation for normalization (Push method in SPM)
deformation = [options.subj.norm.transform.inverseBaseName, 'spm.nii'];
...
for i=1:length(input)
    if isfile(input{i})
        ...
        if mean(gaussdim>1)
            ...
            copyfile(input{i}, tempInputFile);
        end
        ...
        matlabbatch{1}.spm.util.defs.out{1}.push.savedir.saveusr = {outputDir};
        matlabbatch{1}.spm.util.defs.out{1}.push.fov.file = {[ea_space, options.primarytemplate, '.nii']};
        ...
        movefile([outputDir, filesep, 'swr', inputFileName, '.nii'], output{i});

        ea_delete(tempInputFile);
    end
end
