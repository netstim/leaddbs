...
% to a voxel-size of 5.5 mm isotropic and thresholded at 0.5 (threshold was
...
clear
clc
reslice_nii([fileparts(which('lead')),filesep,'templates/mni_icbm152_gm_tal_nlin_asym_09c.nii'],'mni_gm_06.nii',[6,6,6]);
...
V.fname='tmni_gm_06.nii';
V.dt=[16,0];
spm_write_vol(V,X);


V=spm_vol('tmni_gm_06.nii');
X=spm_read_vols(V);
disp(['Sum is ',num2str(sum(X(:))),'.']);

V.fname='mni_parcellation_06_t05.nii';
...
cnt=1;
X(:)=0;
of=fopen('mni_parcellation_06_t05.txt','w');
...
spm_write_vol(V,X);
fclose(of);
of=fopen('mni_parcellation_06_t05.txt');
