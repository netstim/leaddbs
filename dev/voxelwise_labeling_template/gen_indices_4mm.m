...
% to a voxel-size of 4 mm isotropic and thresholded at 0.5 (threshold was
% actually set to 0.52483 to obtain a round number of exactly 35k nodes).
...
clear
clc
ea_reslice_nii('../mni_icbm152_gm_tal_nlin_asym_09c.nii','mni_gm_04.nii',[4,4,4]);
V=spm_vol('mni_gm_04.nii');
X=spm_read_vols(V);
X(X<0.52483)=0;
X=logical(X);
...
V=spm_vol('tmni_gm_04.nii');
X=spm_read_vols(V);
disp(['Sum is ',num2str(sum(X(:))),'.']);

V.fname='mni_parcellation_04_t05.nii';
...
cnt=1;
X(:)=0;
of=fopen('mni_parcellation_04_t05.txt','w');
