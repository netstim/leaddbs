...
if ~ischar(pts)
    ...
    labout=S{1};
    labout.img=in;
    labout.fname=[label,'_firstlevel.nii'];
...
%% part two: iterate the whole brain by samples from this first estimate.
    ...
    for pt=1:size(pts,1)
        for src=1:length(srcs)
       profile(pt,src)=S{src}.img(sub2ind(size(S{src}.img),pts(pt,1),pts(pt,2),pts(pt,3))); 
        end
    end
    
    up=mean(profile,1);
    stp=std(profile,1);
    predictiveweight=(stp./up).^-1; % how constant are values in different acquisitions in the same tissue?
    predictiveweight=predictiveweight./mean(predictiveweight);
    
    for src=1:length(srcs);
        profile(:,src)=((profile(:,src)-up(src))*predictiveweight(src))/stp(src);
    end
    ...
    for ind=1:chunk:dimens
        ...
        for src=1:length(srcs);
            thisindprofile(:,src)=((thisindprofile(:,src)-up(src))*predictiveweight(src))/stp(src);
        end
    ...
    labout=S{1};
    labout.img=1/exp(0.05*Xprob);
    
    labout.fname=[label,'_secondlevel.nii'];
    spm_write_vol(labout,labout.img);
