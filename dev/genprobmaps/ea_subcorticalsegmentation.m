...
if 1
    ...
    srcs{1}=[directory,options.prefs.prenii_unnormalized];
...
cfg_util('run',{matlabbatch});
        srcs{1+m}=[directory,'s',mults(m).name];
    end
    ...
    for s=1:length(structures)
        switch structures{s}
            case {'Pallidum','Thalamus','Striatum','Ruber','STN'}
                ...
                ea_generate_probmaps(pts(1:3,:)',label,srcs);
            case {'wm','gm','csf'}
                label=structures{s};
...
if wtamaps
    ...
    for s=1:length(structures)
        S{s}=ea_load_nii(['s',structures{s},'_secondlevel.nii']);
        A(:,:,:,s)=S{s}.img;
        S{s}.img(:)=nan;
    end
    ...
    for s=1:length(structures)
        S{s}.fname=[structures{s},'_wta.nii'];
