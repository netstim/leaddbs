...
for g=1:length(idlabels)
    ...
    if mean(thissims)>0
        text(double(mean(thissims)),(ax.YLim(2))-(g*(0.07*ax.YLim(2))),[' \leftarrow ',ea_sub2space(idlabels{g}),' [p = ',sprintf('%.4f',p),']'],'Color',ccode(cols(g),:),'FontSize',14,'FontWeight','bold','HorizontalAlignment','left','BackgroundColor','w');
    else
        text(double(mean(thissims)),(ax.YLim(2))-(g*(0.07*ax.YLim(2))),['',ea_sub2space(idlabels{g}),' [p = ',sprintf('%.4f',p),'] \rightarrow '],'Color',ccode(cols(g),:),'FontSize',14,'FontWeight','bold','HorizontalAlignment','right','BackgroundColor','w');
    end
end