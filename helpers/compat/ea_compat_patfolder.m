...
if exist([directory,'stimulations'],'dir') && ~exist([directory,'stimulations',filesep,'template'],'dir') && ~exist([directory,'stimulations',filesep,'native'],'dir')
    di = dir([directory,'stimulations']);
    mkdir([directory,'stimulations',filesep,'native']);
    mkdir([directory,'stimulations',filesep,'template']);
    for d = 1:length(di)
        if di(d).isdir && ~strcmp(di(d).name(1),'.')
        movefile([directory,'stimulations',filesep,di(d).name],[directory,'stimulations',filesep,'template',filesep,di(d).name]);
        end
    end
end
