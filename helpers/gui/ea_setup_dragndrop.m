function ea_setup_dragndrop(handles)

dndcontrol.initJava();

% Create dndcontrol for the JTextArea object

[jhandles,levels,parentIdx,listing] = findjobj(handles.leadfigure);
dndobj = dndcontrol(jhandles(1));


% Set Drop callback functions
dndobj.DropFileFcn = {@DropFcn,handles};




function DropFcn(~,evt,handles)
switch evt.DropType
    case 'file'
        keyboard
        ea_load_pts(handles,evt.Data);
...
function jframe = ea_getjframe(hFig)
