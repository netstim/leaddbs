function M = ea_write_scores(M,nummat,matname,new_var,val_to_rm)
...
if isfield(M,'clinical')
    for pt=1:length(M.patient.list)
        ...
        if exist(fullfile(M.patient.list{pt},'clinical','clinical_scores.mat'),'file')
            ...
            odir = [M.patient.list{pt},'/clinical'];
            mkdir(odir)
        end
        ...
        if ~strcmp(val_to_rm,'') && ~strcmp(val_to_rm,'all')
            ...
            if isfield(scores.(score_type).(postop_flag),(sub_score))
                ...
                if length(fieldnames(scores.(score_type).(postop_flag))) == 1
                    scores = rmfield(scores,score_type);
                end
                save(score_dir,'scores')
                if pt==1
        ...
        elseif strcmp(val_to_rm,'all')
            score_type = fieldnames(scores);
            for i=1:length(score_type)
                scores = rmfield(scores,score_type{i});
